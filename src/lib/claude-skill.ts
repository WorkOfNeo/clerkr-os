...
2. Extract these fields:
   ...
   - \`imageUrl\` — \`<meta property="og:image">\` or the most prominent hero
     ...
     will render text-only. (Optionally pick one from the Abstract image pool
     below if you want visual weight.)
...
1. **Describe** what's visible in the image. Cover:
   ...
   - if the image references a known site, name it
2. **URL handling for the post's \`url\` field**:
   - If the user provided a source URL → use it.
   - If no source URL → ask once: "Do you have a source URL for this, or
     should I file it as a text-only entry?"
   - If image-only → set \`url\` to \`https://unsplash.com/s/photos/abstract\`
     (a stable referent for text-only entries).
3. **imageUrl**:
   - Leave **unset** for text-only cards (the UI handles this fine).
   - **Optionally** pick a URL from the Abstract image pool below for visual
     flavour.
